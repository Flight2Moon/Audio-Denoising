...
class UNetAutoEncoder(nn.Module):
    ...
    def forward(
        ...
        loss = critation_1(label_data, recon)

        if critation_2 is not None:
            # for the ssim l
            loss = loss * loss_weights[0] + critation_2(label_data, recon) * loss_weights[1]

        if critation_3 is not None:
            loss = loss +  critation_3(label_data, recon) * loss_weights[2]
