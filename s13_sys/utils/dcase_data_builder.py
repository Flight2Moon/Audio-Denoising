...
def audio2logmel(raw_audio, sr=16000, n_mels=128, n_fft=2048, hop_length=250):
    # Normalize
    raw_audio = raw_audio / np.max(np.abs(raw_audio))
...
def process_audio_to_npy(audio_path, model, input_size, device, output_dir):
    ...
    input_tensor = F.interpolate(tensor, size=(128, 640), mode="bilinear", align_corners=False)
    ...
    # ✅ Add this line to match DCASE format
    denoised_np = denoised_np.T  # Now shape is (640, 128)
...
def main():
    ...
    input_audio_dir = (
        ...
        f"/home/jihoney/workdir/main_workdir/audio_denoising/dataset/dcase2023_task2_dataset/unzip/dev_data/"
        f"{dataset_audio_class}/train"
    )
    # /home/jihoney/workdir/main_workdir/audio_denoising/dataset/dcase_denoised_dataset/dcase2023_s13_denoised_dataset/bearing
    output_npy_dir = f"/home/jihoney/workdir/main_workdir/audio_denoising/dataset/dcase_denoised_dataset/dcase2023_s13_denoised_dataset/{dataset_audio_class}/train"
    model_ckpt = (
        ...
        f"/home/jihoney/workdir/main_workdir/audio_denoising/workspace/s13_sys/trained_models/dcase2023_dataset_8b_100e_{dataset_audio_class}_13pairs_256x256.pt"
    )
    input_size = 256
